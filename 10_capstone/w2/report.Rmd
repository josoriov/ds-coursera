...
Given that this is only an exploratory data analysis, only an small portion of the files total lines are going to be used. To ensure that this is still representative, the lines to be used are to be sampled randomly. For the blogs and news files 22500 files will be extracted, while for the twitter file 67500 lines are chosen. This is because it is assumed that a twitter entry is much shorter than a news or a blog one, and we want to have similar amount of words for the stats.
...
```{r healt-plot, fig.align='center', fig.width=6, fig.height=4, cache=TRUE, fig.cap="N-gram nomalized cumulative frequency", echo=FALSE}
n1 <- freqReader(1, 500)
...
We see that twitter has the lowest percentage (`r percent$twitter`%) while news has the higher (`r percent$news`%). THe proposed explanation to that is that news articles have a more formal language than Twitter post which are mostly informal and personal.

After that, a table that includes the 15 most frequent bigrams for every file is made. 

```{r wordcloud-plot, cache=TRUE, echo=FALSE}
